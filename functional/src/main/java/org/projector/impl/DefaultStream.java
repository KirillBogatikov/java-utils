...
public class DefaultStream<ValueType> implements Stream<ValueType> {
	...
	@Override
	public <OutType> Stream<OutType> group(Consumer<Duet<ValueType, OutType>, OutType> groupEditor, Consumer<ValueType, Integer> consumer) {
		checkNotNull(consumer, "Consumer");
        ArrayList<OutType> list = new ArrayList<>();
	...
	public Float sumFloat(Consumer<ValueType, Float> consumer) {
		...
		foreach(v -> { sum.set(sum.get() + consumer.consume(v)); });
